# Post-MVP Follow-Ups

Remaining items to hold until explicitly instructed:

- Add picker timeout cleanup that disables expired components or clears service
  state.
- Add narrow live smoke coverage for autocomplete and price shape checks without
  asserting volatile values.
- Clean up or reconcile stale local plan/checklist docs once the branch state is
  settled.
- Revisit optional API-backed features such as card text, variant-aware output,
  and `/sets`.
